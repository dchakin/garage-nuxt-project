...
npm test                     # vitest run — unit + integration (see Testing below)
npm run test:watch           # vitest in watch mode
```

There is no lint script configured in `package.json` as of this writing — don't assume `npm run lint` exists; check `package.json` before relying on it.
...
**Testing:** `vitest.config.ts` defines two projects — `node` (default) for `tests/unit/**`, `tests/server/**`, `tests/integration/**`, and `nuxt` (happy-dom, currently unused) reserved for `tests/nuxt/**` component/composable tests. `tests/setup/node-globals.ts` puts h3's `createError` on `globalThis` so `server/utils/*.ts` files (written assuming Nitro's auto-imports) can be imported directly in tests. `tests/helpers/testDb.ts` creates a throwaway SQLite file per test file (migrated + seeded), since `server/database/index.ts` opens its connection from `DB_FILE_NAME` at import time — set that env var and dynamically `import('../../server/database')` *after* setting it, don't use a static top-level import. API route handlers (`server/api/**`) rely on Nitro/`nuxt-auth-utils` auto-imports (`requireUserSession`, session cookies via `#imports`) that can't be stubbed directly, so those are tested through a real dev server via `@nuxt/test-utils/e2e`'s `setup()` (`tests/integration/api.test.ts`) — note `useTestContext()` (used by that package's `$fetch`/`fetch`) is only populated during an `it()` body (its `beforeEach`/`afterEach`), not inside a `describe`'s own `beforeAll`; do cross-request setup (e.g. logging in) in a leading `it()` instead. Session cookies are stateless (signed, no server-side store), so "logged out" must be tested by resending the *expired* cookie the logout response set, not the original login cookie. `computeNextDue` builds dates via local `Date`/`toISOString()`, so date-related tests must pin `TZ=UTC` (already set in `vitest.config.ts` and in the integration test's server `env`) or results shift by a day depending on the machine's timezone.
